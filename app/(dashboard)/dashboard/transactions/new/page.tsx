...
const NewTransaction = ({ params }: { params: { key: string } }) => {
  const { setVoucherSpecialKey }: any = useGeneralContext();
  ...
  return (
    <>
      <div className="max-w-md md:max-w-full flex flex-col gap-4 justify-start w-full mx-auto">
        {/* breadcumb */}
        ...
        <div className="flex gap-2 items-start justify-between">
          ...
          {!display && (
            <div className="flex flex-col w-full">
              <div className="rounded-xl bg-brand-white p-4 flex flex-col gap-2 justify-start min-h-[80vh]">
                ...
                {/* input fields */}
                <div className="flex flex-col sm:flex-row items-center justify-start gap-6 my-8">
                  <div
                    onClick={() => setDisplay("fund")}
                    className={`transition-fx group w-[240px] max-w-[240px] h-[188px] p-4 flex flex-col justify-start gap-2 border-[0.1px] border-brand-grayish rounded-lg cursor-pointer hover:text-brand-white hover:bg-brand-main ${
                      display === "fund"
                        ? "bg-brand-main text-brand-white"
                        : "text-brand-main bg-brand-white"
                    }`}
                  >
                    <svg
                      width="58"
                ...
                <div className="rounded-b-xl bg-brand-white p-4 flex justify-between items-center border border-brand-grayish/15 mt-auto">
                  <Link
                    ...
                    className="py-1.5 sm:py-3 px-4 sm:px-8 bg-transparent text-[#DE2626] border-[0.3px] border-[#DE2626] font-normal text-base w-max font-geistsans rounded-3xl uppercase cursor-pointer hover:bg-[#DE2626] hover:text-brand-white"
                  >
                    Back
                  </Link>
                </div>
