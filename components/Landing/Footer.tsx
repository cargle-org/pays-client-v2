...
const Footer = () => {
  ...
  const footerNav2 = [
    ...
    { title: "Business on Pays", route: "/business" },
  ];
  ...
  return (
    <div className="w-full bg-brand-gray300">
      <div className="w-full max-w-[1200px] mx-auto py-4 px-6">
        <div className="flex justify-between items-start gap-6 my-14 sm:flex-col sm:items-center sm:gap-10">
          <div>
            <Image
              ...
              width={160}
              height={54}
              className="sm:mx-auto sm:mb-10"
            />
            <p className="my-4 text-xl text-brand-gray500 font-light sm:text-center">
              <span className="font-medium">Usepays</span> offers a seamless
              voucher <br />{" "}
              <span className="text-brand-grayish">
                creation and redemption service
              </span>
            </p>
          </div>
            ...
            <div className="flex flex-col gap-2 items-start mt-2">
              {footerNav1.map((data, index) => (
                <Link
                  ...
                  href={data.route}
                  className={
                    " text-brand-gray200 font-light hover:text-brand-grayish"
                  }
                >
                  {data.title}
                </Link>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 items-start mt-2 sm:flex-row sm:gap-4">
              <Link href={"hi@usepays.co"}>
                <Mail />
              </Link>
              <Link
                href={
                  "https://www.instagram.com/usepays_co?igsh=YXNwODdlMzZiYjI4"
                }
                target="_blank"
                rel="noreferrer"
              >
              ...
              <Link
                href={"https://www.linkedin.com/company/usepays-co/"}
                target="_blank"
                rel="noreferrer"
              >
                {" "}
        ...
        <p className="w-fit mx-auto pt-14 text-brand-gray200">
          © {new Date().getFullYear()} PayS. All Rights Reserved.{" "}
        </p>
      </div>
    </div>
  );
